...
module round_port_curved(d, D, l, straight_l, angle, solid = false){
    wrap_l2 = 25;
    angle_2 = 34;
...
module port_hole(d, D, l = thickness){
    ...
    *resize([D+3,d+3,l*3])
    ...
    translate([0,0,-0.1])
    //rotate([0,0,0])
    polyRoundExtrude(base, l*1.1, 1,-5, $fn=100);
    *resize([D+7, d+7, 3])
        cylinder(h=3, d = d + 7);
}
...
module c8_placeholder(y, z, angle){
    width = 35.5;
    height = 14.4;
    w = width/2;
    h = height/2;
    ...
    module extruder(w, h, r, d){
        ...
        polyRoundExtrude(b, d,0,0, $fn=100);
    }
    ...
    translate([0,y,z])
    rotate([angle, 0,0]){
    extruder(c_width/2, c_height/2, c_r, c_depth);
    extruder(w, h, r, depth);
        
    for(i=[0,180]){
        rotate([0,0,i])
        translate([14.05,0,depth])
        cylinder(h = thickness, d = 3.2);
    }
    *translate([0,0,-2])
    extruder(w, h, r, depth);
   } 
...
module main_case(){
    difference(){
        ...
        rotate([0,0,180]){
            deepness = 10;
            difference(){
                cylinder(h=deepness, d = 94);
                *translate([0,55,0])
                cube([70,20,20], center = true);
            }
            translate([0,0,deepness])
            woofer();
        }
        //port    
        translate([0,(h-zmove_port)/angle_tan+ bot_r/1.2 - 40/cos(minus_angle),zmove_port])
        ...
        union(){
            ...
            c8_placeholder(-(bot_r), -0,-90);
            //power_cord_input(-bot_r+thickness+1, 3,0);
        }
        translate([0,64.6,zmove_port])
        rotate([angle,0,0])
        round_port_curved(d1, D, port_l, port_str_l, angle=port_angle, solid=true);
    }
    difference(){
    translate([0,64.6,zmove_port])
        rotate([angle,0,0])
